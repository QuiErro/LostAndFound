<template>
  <div class="goods">
    ...
    <div class="goods-content">
      <div class="post-img" v-if="showImage && selected_found_goods.picture">
        <yd-slider autoplay="2500" speed="1000">
          <yd-slider-item v-for="(pic, index) in selected_found_goods.picture" :key="index">
      ...
      <div class="post-detail">
        <h4>详细信息</h4>
        <div>{{selected_found_goods.info || '暂无'}}</div>
      </div>
...
<script>
  ...
  export default {
    ...
    watch:{
      $route(){
        if(this.$route.path.includes('/mypostfoundgoods')){
          if(this.selected_found_goods.picture){
            this.showImage = true;
          }
          this.lat = this.selected_found_goods.latitude;
          this.lng = this.selected_found_goods.longitude;
